...
hosts_temp = "hosts"
redirect = "127.0.0.1"
web_sites_list = ["www.facebook.com", "facebook.com"] # users can modify the list of websites they want to block
...
while True:
    if dt(dt.now().year, dt.now().month, dt.now().day, 9) < dt.now() <dt(dt.now().year, dt.now().month, dt.now().day,22):
        ...
        with open(hosts_path, "r+") as file:
            ...
            file.truncate() # this line is used to delete the training lines that contains DNS
