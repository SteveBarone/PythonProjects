...
def game():
    ...
    # loop for destroying the window after on test
    if x == 0:
        window.destroy()
        x = x+1

    # defining function for results of the test
    def check_result():
        if entry.get() == words[word]:
            # here start time is when the window is opened and end time is when
            # window is destroyed
            end = timer()

            # we deduct the start time from end time and calculate results
