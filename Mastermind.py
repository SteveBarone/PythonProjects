...
if (n == num):
    ...
    # ctr variable initialized. It will keep count of the number of tries
    # the player takes to guess the number.
    ctr = 0

    # while loop repeats as long as the player fails to guess the number correctly.
    while (n != num):
        # variable increments every time the loop is executed, giving an idea
        # of how many guesses were made.
        ctr += 1

        count = 0

        # explicit type conversion of an integer to a string in order to ease
        ...
        for i in range(0, 4):
            ...
            if (n[i] == num[i]):
                ...
                count += 1
                # hence, the digit is stored in correct[1]
                correct[i] = n[i]
