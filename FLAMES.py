    ...
    for i in range(len(list1)):
        for j in range(len(list2)):
            ...
            if list1[i] == list2[j]:
                ...
                # concatenation of two list elements with *, * acts as border mark
                list3 = list1 + ["*"] + list2

                # return the concatenated list with True flag
                return [list3, True]

        # no common characters found, return the concatenated list with False flag
    list3 = list1 + ["*"] + list2
    return [list3, False]
...
if __name__ == "__main__":
    ...
    # keep calling the remove_match_char function until common characters is found
    ...
    # keep looping until only 1 item is not remaining in the return list
    while len(result) > 1:
        # store that index value from where we have to perform slicing
        split_index = (count % len(result) -1)

        # this step is done for performing anti clockwise circular fashion counting.
        if split_index >= 0:
            # list slicing
